...
windProfile=char(typeWind{2}); % power or log or NPD
...
for ii=1:Nfreq
    ...
    if strcmpi(type,'NPD')
        randPhase = rand(Nm,1); % set random phase
    ...
    elseif strcmpi(type,'NPD')
        [Su] = NPDSpectrum(f(ii),Uref,nodes.Z);
    else
    ...
    if strcmpi(type,'NPD')
        Suu = sqrt(Su*Su').*cohU;
        [L,D]=ldl(Suu,'lower'); % a LDL decomposition is applied this time
    else
        Suu = sqrt(Su*Su').*cohU;
        Svv = sqrt(Sv*Sv').*cohV;
        Sww = sqrt(Sw*Sw').*cohW;
    ...
    function [nodes,Nm] = createGrid(geometry,filename)
        if isempty(geometry)
            ...
            elseif strcmpi(windProfile,'log')
                ...
                U = u_star./k.*log(z./z0);
            elseif strcmpi(windProfile,'NPD')    
                U = Uref.*(1+0.0573 * sqrt(1+0.15.*Uref)*log(z/10));
            else
